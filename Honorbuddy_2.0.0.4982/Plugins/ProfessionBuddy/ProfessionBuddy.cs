...
namespace HighVoltz
{
    public partial class Professionbuddy : HBPlugin
    {
        ...
        public override Version Version { get { return new Version(1, 0, 0, 35); } }
        ...
        public void UpdateMaterials()
        {
            try
            {
                ...
                {
                    MaterialList.Clear();
                    foreach (IPBComposite pa in CurrentProfile.Branch.Children)
                    {
                        if (pa is CastSpellAction)
                        {
                            CastSpellAction ca = (CastSpellAction)pa;
                            if (ca.IsRecipe)
                            {
