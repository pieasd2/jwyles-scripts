...
namespace HighVoltz
{
    public partial class Professionbuddy
    {
        ...
        public void GenorateDynamicCode()
        {
            ...
            foreach (IPBComposite composite in CurrentProfile.Branch.Children)
            {
                if (composite is If || composite is CustomAction || composite is WaitAction)
                    StoreMethodName(composite);
            }
        ...
        void StoreMethodName(IPBComposite comp)
        {
            if (comp is If)
            {
                DecoratorMethods["CanRun" + Util.Rng.Next(int.MaxValue).ToString()] = (If)comp;
                foreach (IPBComposite child in ((PrioritySelector)((Decorator)comp).DecoratedChild).Children)
                {
                    if (child is If || child is CustomAction || child is WaitAction)
