...
namespace HighVoltz.Composites
{
    ...
    class MailItemAction : PBAction
    {
            ...
            if (!IsDone)
            {
                ...
                else
                {
                    ...
                    using (new FrameLock())
                    {
                        ...
                        Lua.DoString(string.Format("SendMail ('{0}',' ','');SendMailMailButton:Click();", LevelbotSettings.Instance.MailRecipient));
                    }
                    if (IsDone)
                    {
        ...
        public override string Title
        {
            get
            {
                return string.Format("{0}: to:{1} {2} ", Name, LevelbotSettings.Instance.MailRecipient, 
